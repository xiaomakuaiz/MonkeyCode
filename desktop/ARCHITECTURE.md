...
HTTP API、附件直传、工作区上传/下载与登录探测共用 `Service::send_request`：
调用方选择请求体/响应格式和时限，公共层处理 TLS、凭证与响应 Cookie 更新。
预签名上传仅对与主服务同源(协议/主机/有效端口相同)的 URL 附加匹配的
Cookie 和浏览器身份，不追加 `Authorization`；外部存储不携带本站凭证，
也不吸收其 Cookie。发送前的会话代次校验与 Cookie 读取在同一锁内完成，
登出/切服后的旧快照不能再发起请求(包括 presign 后的 PUT)，不能把新会话
凭证送往旧地址；锁不跨网络等待。响应刷新也沿用代次守卫，迟到响应不能
恢复旧会话。百智账号罐保持独立；登录探测只用临时 Cookie，不读写持久罐。

云端用户消息的附件地址以 WS/历史 `user-input.attachments` 为准，不用本地上传
引用覆盖。图片缩略图与大图经 `mc_attachment_read` 读取：相对资源路径按云端
服务解析，认证沿用公共请求层，UI 仅收到不含凭证的图片 data URL。外部存储
不携带本站凭证；读取绑定 transport/会话代次、禁止自动跳转、限制 20MB，
按文件头识别图片并拒绝登录页 HTML，错误在图片位置提供重试。
