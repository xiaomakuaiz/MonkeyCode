...
- 选中 = menu-active;attention(D3 未读)= warning 状态点 + bg-warning/10
  行淡底(功能性状态色,§8 白名单)。
- **文字色阶三档**(2026-08-07 对表旧 UI sidebar.tsx 定案):组头满色
  (锚点)> 行 `/90`(旧 UI `--t2` 92% 同档)> **已归档行 `/55`**
  (旧 UI `--t4`)。归档行此前用正文色,在列表里和活跃任务一样抢眼
  (用户报障「已归档的任务标题怎么还是黑色的」)。选中态不降档——选中
  就该看清,颜色交回 `menu-active-fg`。归档**项目**组头连同图标降一档。
...
- 项目组头 = details summary,**锚点形态**(2026-08-07 对表旧 UI 定案,
  推翻 08-04「区块标签 = 小一档 + /50」):summary 用 flex 覆写默认 grid,
  项目图标(Folder 13px /65)+ 名称(**与行同字号 text-sm + font-semibold
  + 满色**;**保留原大小写**——目录名是标识符,不做 uppercase,用户定案
  2026-08-04)flex-1 伸展 + waiting 徽标 +
  ...
  border-t 指示线。
- **层级靠「组头字重 + 缩进 + 引导竖线」,不靠空白**(2026-08-07 用户
  四轮报障后定案:①「亲密性不够,像一路排下来」②「项目之间太空了」
  ③「看看别人类似组件怎么设计的」④「间距还是不对…看看之前 UI 是咋搞的」)。
  > **④ 的收敛是最要紧的一条**:对表旧 UI 后发现,让任务「挂」在项目下面
  > 的**不是间距,是强调关系**——旧 UI 组头 12.5px/**600**/`--t1`(全列
  > 最深),任务行 12.5px/400/`--t2`;ui-next 此前恰好做反(组头最淡最小、
  > 行最深最大),项目名成了飘在一堆大黑字上方的说明文字,于是间距怎么调
  > 都不亲。**先把强调关系摆正,再谈间距**(色阶三档见 §6.1)。
