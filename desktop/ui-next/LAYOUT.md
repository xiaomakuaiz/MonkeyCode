...
- 项目组头 = details summary,**区块标签形态**(定案 2026-08-04 二次):
  ...
  border-t 指示线。
- **组内亲密性靠三档间距分层**(2026-08-07 用户报障「亲密性不够,像一路
  排下来」):daisyUI 给每行/组头 `padding-block:.375rem`,行盒本身是紧挨
  的(盒间空白恒 0),**肉眼的间距 = 上盒下内距 + 下盒上内距**。原先
  组头→首行 12px、行→行 12px、组→组 16px(`mt-1`)三档几乎等距,层级只
  剩缩进一个信号。现:组头 `pb-0.5` 贴住自己的首行(12→**8px**)、组 li
  `mt-4`(16→**28px**),行→行保持 12px 不动(命中区不缩水)——
  8 < 12 < 28,组间是行间的 2.3 倍,组才成其为组。小节头(已归档任务/
  历史任务)同取 `pb-0.5`。**别只加组间距**:2026-08-05「`mt-2` 太散」的
  旧定案正是因为当时没有组头贴行这一档,加空白只让整列变稀,不产生分组。
