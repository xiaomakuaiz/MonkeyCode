...
- **「填充」这个通道只归选中**(2026-08-10 用户定案):选中 = `menu-active`
  (primary 12% 淡填充);attention(D3 未读/等你处理)= **行左缘 2px warning
  竖条**(`listKit.ATTENTION_BAR`,绝对定位不挤布局)+ 行尾 warning 状态点。
  ...
  > 组件与邮件客户端同款分工)。
  > 条子挂**行左缘**而非缩进后的文字前:各层级的待办行因此对齐在同一条 x 上,
  > 一眼扫得出「有几件事在等我」。**别再把 attention 改回整行底色。**
...
- 统一视觉调整放到收尾,一次性经**主题变量**(monkeycode 主题块)完成,
  不散落在组件类上。
- 允许保留:功能性状态色(attention 行的 warning 左缘条、状态点)、文字四档
