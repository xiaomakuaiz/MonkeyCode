...
- 项目组头 = details summary,**区块标签形态**(定案 2026-08-04 二次):
  ...
  border-t 指示线。
- **组内亲密性靠三档间距分层,且间距一律挂「展开态」**(2026-08-07 用户
  两轮报障:①「亲密性不够,像一路排下来」②「项目之间太空了」)。
  daisyUI 给每行/组头 `padding-block:.375rem`,行盒本身紧挨(盒间空白恒 0),
  **肉眼间距 = 上盒下内距 + 下盒上内距**。原先组头→首行 / 行→行 / 组→组
  三档几乎等距(约 7/10.5/14px),层级只剩缩进一个信号。现:
  | 关系 | 值 | 载体 |
  |---|---|---|
  | 组头→首行 | 7px | 组头 `pb-0.5`(收掉下内距贴住首行) |
  | 行→行 | 10.5px | 不动(命中区不缩水) |
  | 末行→下一组头 | 24.5px | `<details>` 的 `open:mb-4` |
  | 组头→组头(均折叠) | 10.5px | 无附加间距 |
  > **两条都只在展开时生效**:折叠的组没有要分开的内容、也没有要贴的行,
  > 组头退化成普通标签行,按列表统一节奏排——常驻组间距会让「全部折叠」
  > 的侧栏变成一列飘着的标签(第②轮报障即此)。故 `mb` 走 `open:` 变体、
  > `pb-0.5` 由折叠态开关,**不要**把间距挂在组 `li` 上。小节头(已归档
  > 任务/历史任务)与云端项目组头同此一套(三列表一套件)。
  > **也别只加组间距**:2026-08-05「`mt-2` 太散」的旧定案正是因为当时缺
  > 「组头贴行」这一档,光加空白只让整列变稀,不产生分组。
