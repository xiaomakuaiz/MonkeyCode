...
- 项目组头 = details summary,**区块标签形态**(定案 2026-08-04 二次):
  ...
  border-t 指示线。
- **组内缩进阶梯:每级 = 图标宽 12px**(2026-08-05 用户定案三次,推翻
  08-04「组内不缩进」与同日的 menu 原生 1.5rem 方案):嵌套 ul 一律
  `ms-3 ps-0`;行首标记统一 12px 定宽槽(状态点/Archive/Folder 同列),
  同级行文字因此对齐;层级链 = 项目头 → 任务行 → 「已归档任务」小节头
  (Archive 图标行首、`after:hidden` 去 menu 尾箭头)→ 归档行;嵌套竖线
  仍隐藏(before:hidden);云端项目组同构。
