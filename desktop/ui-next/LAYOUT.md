...
- 项目组头 = details summary,**区块标签形态**(定案 2026-08-04 二次):
  ...
  border-t 指示线。
- **组内层级 = menu 原生嵌套缩进**(2026-08-05 用户定案,推翻 08-04 的
  「组内不缩进」:四层顶同一条左线读不出归属,回归旧 UI 缩进阶梯):
  嵌套 ul 不再加 ms-0/ps-0,吃 daisyUI 原生每级 1.5rem(1rem margin +
  0.5rem padding);项目头 → 任务行一级 → 项目内「已归档任务」小节头同
  任务行 → 归档行再进一级;嵌套竖线仍隐藏(before:hidden,既有指令);
  云端项目组同构。
