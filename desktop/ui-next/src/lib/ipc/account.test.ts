...
describe("account 契约:MonkeyCode 命令名与载荷形状", () => {
  ...
  it("mc_login / mc_checkin / mc_models_sync:命令字面量", async () => {
    const calls = stubInvoke((cmd) =>
      Promise.resolve(cmd === "mc_models_sync" ? { models: [{}], notes: [] } : { ok: true }),
    );
    await mcLogin();
    await mcCheckin();
    expect((await mcModelsSync()).models).toHaveLength(1);
    expect(calls.map((c) => c.cmd)).toEqual(["mc_login", "mc_checkin", "mc_models_sync"]);
  });
});

describe("disconnectMc:先 revoke 再 logout 的顺序", () => {
  it("正常断开:mc_models_revoke 先于 mc_logout,无 warning", async () => {
    const calls = stubInvoke(() => Promise.resolve({ ok: true }));
    expect(await disconnectMc()).toEqual({});
    expect(calls.map((c) => c.cmd)).toEqual(["mc_models_revoke", "mc_logout"]);
  });

  it("revoke 失败不阻断登出:仍走 mc_logout,失败信息进 warning", async () => {
    const calls = stubInvoke((cmd) =>
      cmd === "mc_models_revoke" ? Promise.reject(new Error("网络不可达")) : Promise.resolve({ ok: true }),
    );
    expect(await disconnectMc()).toEqual({ warning: "网络不可达" });
    expect(calls.map((c) => c.cmd)).toEqual(["mc_models_revoke", "mc_logout"]);
  });

  it("logout 本身失败照常上抛", async () => {
    stubInvoke((cmd) =>
      cmd === "mc_logout" ? Promise.reject(new Error("登出失败")) : Promise.resolve({ ok: true }),
    );
    await expect(disconnectMc()).rejects.toThrow("登出失败");
  });
});
