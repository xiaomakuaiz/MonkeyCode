...
function mcpParts(tool: string): { service: string; operation: string } | null {
  ...
  if (parts.length < 3 || !head || !service || head.toLowerCase() !== "mcp") return null;
  return { service, operation: parts.slice(2).join("_") };
}
...
function structuredTarget(
  ...
  toolKind?: string,
  meta?: unknown,
): { target: string; kind: ToolTargetKind } | null {
  ...
  if (mcp && ["mc-browser", "browser", "playwright"].includes(mcp.service.toLowerCase())) {
    switch (mcp.operation) {
      ...
      case "browser_take_screenshot":
        return { target: inputValue(input, "full_page") === "true" ? (zh ? "整页" : "full page") : "", kind: "text" };
      case "browser_click":
  ...
  switch (rawTool) {
    ...
    case "ReportFindings": {
      ...
      if (!input.findings.length) return { target: zh ? "未发现问题" : "no findings", kind: "text" };
      return { target: zh ? `${input.findings.length} 项发现` : `${input.findings.length} findings`, kind: "text" };
    }
  }
...
export function presentToolCall(
  ...
  const structured = structuredTarget(base.rawTool, rawInput, locale, toolKind, meta);
