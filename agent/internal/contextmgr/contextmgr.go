...
// PlatformRule 平台下发的规则(内容内联进系统提示)。
type PlatformRule struct {
	Name    string
	Content string
}

// PlatformSkill 平台下发并已落盘的技能索引。
type PlatformSkill struct {
	Name        string
	Description string
	Doc         string // 入口文档绝对路径(可能为空)
	Dir         string // 技能目录
}

// Extras 平台下发的系统提示增量(规则热更新 + 技能索引),nil 表示无。
type Extras struct {
	Rules  []PlatformRule
	Skills []PlatformSkill
}

// Build 组装完整系统提示。extras 为平台下发增量,可为 nil。
func Build(workdir string, extras *Extras) string {
	...
	if extras != nil && len(extras.Rules) > 0 {
		b.WriteString("\n# 平台规则(由 MonkeyCode 平台下发,必须遵守)\n")
		total := 0
		for _, r := range extras.Rules {
			content := r.Content
			if total+len(content) > rulesMaxBytes {
				content = content[:max(0, rulesMaxBytes-total)] + "\n...[平台规则过长已截断]"
			}
			fmt.Fprintf(&b, "## %s\n%s\n", r.Name, content)
			total += len(content)
			if total >= rulesMaxBytes {
				break
			}
		}
	}
	...
	if extras != nil && len(extras.Skills) > 0 {
		b.WriteString("\n# 平台技能(按需使用)\n")
		b.WriteString("以下技能由 MonkeyCode 平台下发并已缓存到本地。当任务与某技能相关时,先用 read_file 阅读其入口文档再按文档执行;不相关则忽略。\n")
		for _, s := range extras.Skills {
			doc := s.Doc
			if doc == "" {
				doc = s.Dir
			}
			fmt.Fprintf(&b, "- %s: %s(文档: %s)\n", s.Name, s.Description, doc)
		}
	}
