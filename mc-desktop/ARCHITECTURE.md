...
(browser_ext/worktree/usage_update/perm_remember/attachments)。
...
会话中途切模型/模式(现 destroy+resume 带全参数)、permission remember
(现壳记忆集自动应答)、sendMessage 附件(现 [图片] 路径文本约定)、
usage 事件(现 UI 隐藏用量条)、每模型独立凭据(现同 configKey 冲突跳过)、
mcp headers(现带鉴权头条目跳过)、stdio 模式会话索引(现 sidecar 权威)。

## 开发

```bash
cd ui && npm run build      # UI 产物 → uidist(入库,cargo build 不依赖 node)
